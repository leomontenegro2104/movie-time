...
const Header: React.FC = () => {
  ...
  return (
    <div
      ref={headerRef}
      className="fixed top-0 left-0 w-full h-20 z-50 transition-all duration-300 bg-transparent shrink:bg-black"
    >
        ...
        <ul className="hidden md:flex items-center space-x-8 text-lg font-semibold">
          {headerNav.map((e, i) => (
            <li key={i} className="relative">
              <Link
                to={e.path}
                className={`text-white pb-1 relative ${i === active ? 'after:w-full' : 'after:w-0'
                  } after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:h-[2px] after:bg-red-500 after:transition-all after:duration-500 hover:after:w-full`}
              >
                {e.display}
              </Link>
            </li>
